...
/* ── Textarea that focuses itself after mount ── */
...
export default function PdfEditor() {
  ...
  const wrapperRef    = useRef(null)   // the div that holds the canvas + overlays
  const scrollAreaRef = useRef(null)
  const didDragRef    = useRef(false)  // true if mousedown→mousemove happened (drawing)
  ...
  /* ── Render page — fills available width ── */
  ...
  /* ── Draw live freehand on overlay canvas ── */
  useEffect(() => {
  ...
  const pushHistory = useCallback((newAnnots) => {
    setHistory(prev => {
      const stack = prev.slice(0, histIdx + 1)
  ...
  const undo = useCallback(() => {
    if (histIdx <= 0) return
    setHistory(prev => {
      setAnnotations(JSON.parse(JSON.stringify(prev[histIdx - 1])))
      return prev
    })
    setHistIdx(i => i - 1)
  }, [history, histIdx])
  ...
  /* ── Get position inside the canvas wrapper ── */
  const getPos = (e) => {
    // Always use the wrapper div (wrapperRef) as the reference — most reliable
    const r = wrapperRef.current.getBoundingClientRect()
    return { x: Math.round(e.clientX - r.left), y: Math.round(e.clientY - r.top) }
  }

  /* ── Mouse handlers for draw tools (highlight, whitebox, draw) ── */
  const onMouseDown = (e) => {
  ...
  /* ── Click handler for text + select tools ── */
  const onClick = (e) => {
    if (e.target.closest('.annot-el')) return
    if (didDragRef.current) return  // was a draw operation, not a click
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      for (const [ps, annots] of Object.entries(annotations)) {
        ...
        const cw = baseVp.width  * scale
        const ch = baseVp.height * scale
        const sx = pw / cw
        const sy = ph / ch
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { icon: '🔤', label: 'Add Text',  desc: 'Click anywhere to place text' },
      ...
      <div style={{ background: '#1e293b', padding: '0 12px', display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, height: 52, overflowX: 'auto' }}>
        {TOOL_LIST.map(t => (
          <button key={t.id} onClick={() => { setTool(t.id); setSelectedId(null); setEditingId(null) }}
            style={{ padding: '5px 12px', borderRadius: 7, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 700, whiteSpace: 'nowrap', background: tool === t.id ? '#3b82f6' : 'rgba(255,255,255,0.08)', color: tool === t.id ? '#fff' : '#94a3b8' }}>
        ...
        <span style={{ fontSize: 11, color: '#475569', marginLeft: 4, whiteSpace: 'nowrap' }}>
          ...
          {tool === 'whitebox'  && '← Drag to cover an area'}
          {tool === 'draw'      && '← Drag to draw freely'}
          {tool === 'select'    && '← Click an annotation to select, × to delete'}
        </span>

        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          <button onClick={undo} disabled={histIdx <= 0} title="Undo (Ctrl+Z)"
            style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #334155', background: 'rgba(255,255,255,0.06)', color: histIdx <= 0 ? '#374151' : '#94a3b8', cursor: histIdx <= 0 ? 'not-allowed' : 'pointer', fontSize: 12, fontWeight: 600 }}>
            ↩ Undo
          </button>
          ...
          {pageAnnots.filter(a => a.type === 'highlight' || a.type === 'whitebox').map(a => {
            ...
            return (
              <div key={a.id} className="annot-el"
                style={{ position: 'absolute', left: a.x, top: a.y, width: a.w, height: a.h, zIndex: 9, pointerEvents: 'all', cursor: tool === 'select' ? 'pointer' : 'inherit', background: a.type === 'highlight' ? 'rgba(253,224,71,0.45)' : '#ffffff', outline: sel ? '2px dashed #3b82f6' : 'none' }}
            ...
            return (
              <svg key={a.id} className="annot-el"
                ...
                <polyline
                  ...
                  stroke={a.color} strokeWidth={sel ? a.lineWidth + 2 : a.lineWidth}
              ...
              <div key={a.id} className="annot-el"
                ...
                {isEd ? (
                  ...
                  <div
                    ...
                    onClick={e => {
                      e.stopPropagation()
                      if (tool === 'text' || tool === 'select') setEditingId(a.id)
                    }}
                    style={{ fontSize: a.fontSize, color: a.color, cursor: 'text', whiteSpace: 'pre-wrap', padding: '3px 6px', border: sel ? '2px dashed #3b82f6' : '1.5px dashed rgba(59,130,246,0.35)', borderRadius: 4, background: sel ? 'rgba(219,234,254,0.15)' : 'transparent', minWidth: 40, lineHeight: 1.4 }}>
                    {a.text}
                  </div>
                )}
                {(sel || isEd) && (
